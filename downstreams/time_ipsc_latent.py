...
from src.models.setscae_module import SetSCAELitModule
from src.models.components.ae.set_scae import SetSCAE

# ============================================================================ 
# Model Loading Helpers
# ============================================================================ 
def load_ae_model(ae_ckpt_path, device):
    print(f"Loading AE model from {ae_ckpt_path}...")
    
    # Try to infer if it's 'large' or 'small' based on path or just use large as default
    # n_input=28231, n_hidden=4096, n_latent=1024, n_layers=6 (Large)
    # n_input=28231, n_hidden=256, n_latent=64, n_layers=2 (Small/Default)
    
    is_large = "large" in ae_ckpt_path.lower()
    
    if is_large:
        print("Detected 'large' AE config.")
        net = SetSCAE(
    ...
    ae_model = SetSCAELitModule.load_from_checkpoint(ae_ckpt_path, net=net, map_location=device)
    ae_model.eval()
    ae_model.to(device)
    return ae_model
...
def check_ae_reconstruction(ae_model, adata, transform_fn, device, out_dir):
    print("\n--- Checking AE Reconstruction Baseline ---")
    os.makedirs(out_dir, exist_ok=True)
    
    # Sample random cells
    indices = np.random.choice(adata.n_obs, min(2000, adata.n_obs), replace=False)
    
    try:
        x_raw = adata.layers["log_normalised"][indices]
    except:
        x_raw = adata.X[indices]
        
    x_np = transform_fn(x_raw)
    x_gene = torch.from_numpy(x_np).to(device)
    
    with torch.no_grad():
        # Encode
        x_set = x_gene.unsqueeze(1)
        z = ae_model.get_latent(x_set)
        
        # Decode
        lib_size = torch.expm1(x_gene).sum(dim=-1).clamp(min=1.0)
        decoded = ae_model.net.decode_cell(z, lib_size)
        x_recon = torch.log1p(decoded['mu'])
        
    x_recon_np = x_recon.cpu().numpy()
    x_true_np = x_np
    
    metrics = calculate_metrics(x_recon_np, x_true_np, out_dir, label_suffix="ae_recon")
    print(f"AE Reconstruction Metrics: {metrics}")
    
    pd.DataFrame([metrics]).to_csv(os.path.join(out_dir, "ae_metrics.csv"), index=False)

def run_inference(model, adata, transform_fn, device, out_dir, scale_factor=1.0, ae_model=None):
    os.makedirs(out_dir, exist_ok=True)
    model.eval()
    ...
    for i in range(len(days) - 1):
        ...
        for b_start in range(0, len(idx_curr), batch_size):
            ...
            x_curr_gene = torch.from_numpy(x_curr_np).to(device)
            
            # [Latent Mode]: Encode to latent space
            if ae_model is not None:
                with torch.no_grad():
                    # SetSCAE expects (B, set_size, n_genes), we use set_size=1
                    x_curr_set = x_curr_gene.unsqueeze(1)
                    z_curr = ae_model.get_latent(x_curr_set)
                    x_curr_flow = z_curr
            else:
                x_curr_flow = x_curr_gene / scale_factor
            
            B = x_curr_flow.shape[0]
            
            # Meta
            cond_data = {
                'x_curr': x_curr_flow,  # Use latent or scaled gene expression
            ...
            # Sample (Prediction)
            with torch.no_grad():
                x_pred_flow = model.flow.sample(x_curr_flow, cond_data, steps=50)
                
            # [Latent Mode]: Decode back to gene space
            if ae_model is not None:
                with torch.no_grad():
                    # Estimate library size from source
                    lib_size = torch.expm1(x_curr_gene).sum(dim=-1).clamp(min=1.0)
                    decoded = ae_model.net.decode_cell(x_pred_flow, lib_size)
                    # Use log1p of mu (reconstructed mean) as the predicted log-normalized expression
                    # Or use rho if we want library-size independent, but metrics use log-normalized.
                    x_pred = torch.log1p(decoded['mu'])
            else:
                x_pred = x_pred_flow * scale_factor
...
def main():
    ...
    # Load checkpoint to check hparams
    ckpt = torch.load(args.ckpt_path, map_location='cpu')
    hparams = ckpt.get("hyper_parameters", {})
    mode = hparams.get("mode", "raw")
    print(f"Model mode: {mode}")
    
    # Load AE if in latent mode
    ae_model = None
    if mode == "latent":
        ae_ckpt = args.ae_ckpt_path or hparams.get("ae_ckpt_path")
        if ae_ckpt is None:
            raise ValueError("AE checkpoint path must be provided for latent mode flow models.")
        ae_model = load_ae_model(ae_ckpt, device)
        input_dim = 1024 # Latent dimension for 'large' AE
    else:
        input_dim = 28231 # Gene expression dimension
    ...
    # Get scale factor from hparams
    scale_factor = getattr(model.hparams, 'scale_factor', 10.0)
    print(f"Using scale factor: {scale_factor}")
    
    if ae_model is not None:
        check_ae_reconstruction(ae_model, adata, transform_fn, device, os.path.join(base_out_dir, "ae_check"))
    
    # --------------------------------------------------------- 
    # Zero-Shot Inference
    # --------------------------------------------------------- 
    print("\n--- Running Zero-Shot Inference ---")
    run_inference(model, adata, transform_fn, device, os.path.join(base_out_dir, "zeroshot"), scale_factor, ae_model=ae_model)
    ...
    if args.fine_tune_epochs > 0:
        ...
        for epoch in range(args.fine_tune_epochs):
            ...
            for batch in tqdm(dataloader, desc=f"Epoch {epoch+1}/{args.fine_tune_epochs}"):
                ...
                # [Latent Mode]: Encode to latent space for training
                if ae_model is not None:
                    with torch.no_grad():
                        batch['x_curr'] = ae_model.get_latent(batch['x_curr'].unsqueeze(1))
                        batch['x_next'] = ae_model.get_latent(batch['x_next'].unsqueeze(1))
        ...
        run_inference(model, adata, transform_fn, device, os.path.join(base_out_dir, "finetuned"), scale_factor, ae_model=ae_model)
